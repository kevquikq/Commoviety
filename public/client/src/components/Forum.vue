          ...
          <div class="panel panel-default">
                ...
                <div class="col-12 msg_container_base">
                    <div class="col-12 msg_container">
                        <div class="col-12">
                            <div :class="getClassMessage(message.userId)" v-for="(message, index) in messages.data.slice().reverse()" :key="index">
                              <button v-if="esUsuarioActual(message.userId)" class="btn btn-sm btn-danger float-right" style="height:28px; margin-top:3px" @click="eliminarMensaje(message.id)">Eliminar</button>
                                <div class="msg rounded mt-1"> <p class="ml-2 mr-2 msg_text">{{message.text}}</p> </div>
  ...
  export default  {
    ...
    methods: {
      ...
      async eliminarMensaje(id)  {
        try {
                await this.axios.delete(`${this.urlBorrar}/${id}`, {'content-type':'application/json'} )
